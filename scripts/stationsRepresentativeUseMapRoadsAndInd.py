...
def build_map_station_nearest_industry_by_pollutant(
    ...
    Mapa Folium mostrando:
      ...
      - Ruas mais próximas (fixas, sempre visíveis).
      - Indústrias mais próximas (fixas, sempre visíveis).
    ...
    # === Camada global de ruas (fixa) ===
    layer_roads = folium.FeatureGroup(name="Ruas mais próximas", show=True).add_to(m)
    ...
    # === Camada de conexões estação ↔ indústria (fixa) ===
